...
const Email = ({ fullName, email, phone, style, message }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Style Book inquiry{style ? ` — ${style}` : ''}{fullName ? ` from ${fullName}` : ''}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Text style={eyebrow}>BEAU MONDE BUILDERS</Text>
        <Heading style={h1}>Style Book Inquiry</Heading>

        {style ? (
          <Section style={highlight}>
            <Text style={highlightLabel}>Style of Interest</Text>
            <Text style={highlightValue}>{style}</Text>
          </Section>
        ) : null}
        ...
        <Hr style={hr} />
        <Text style={footer}>Sent from the Beau Monde Builders Style Book.</Text>
      </Container>
    </Body>
  </Html>
)
...
export const template = {
  ...
  previewData: {
    ...
    phone: '(561) 000-0000',
    style: 'Anglo-Caribbean',
    message: 'Drawn to the verandas and louvered shutters.',
  },
...
const highlight = { backgroundColor: '#0f2a3d', padding: '20px 28px', marginBottom: '16px' }
const highlightLabel = { color: '#b08a4a', fontSize: '10px', letterSpacing: '0.3em', textTransform: 'uppercase' as const, margin: 0 }
const highlightValue = { color: '#f5f0e6', fontSize: '20px', margin: '6px 0 0', fontFamily: 'Georgia, "Cormorant Garamond", serif', fontStyle: 'italic' }
